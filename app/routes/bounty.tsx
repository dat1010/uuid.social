...
export async function loader({ request, context, params }: Route.LoaderArgs) {
  ...
  const [user, object, connections, bounty] = await Promise.all([
    ...
    db.prepare(`SELECT rule_type, ends_at, EXISTS(SELECT 1 FROM bounty_claims WHERE bounty_id = ?) AS claimed FROM bounties WHERE id = ?`).bind(id, id).first<{ rule_type: "character_count" | "event_gap"; ends_at: number; claimed: number }>(),
  ]);
  if (!object || object.type !== "bounty" || !bounty) throw new Response("Not Found", { status: 404 });
  return { signedIn: Boolean(user), object, connections, needsTwo: bounty.rule_type === "event_gap", active: bounty.ends_at > Date.now() && !bounty.claimed };
}
